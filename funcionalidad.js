...
let isOfflineMode = false; // Por defecto asumimos que puede fallar
...
// --- INICIALIZACIÓN ROBUSTA (DINÁMICA) ---
async function initApp() {
    updateConnectionStatus(true); // Empezamos asumiendo offline mientras carga

    try {
        // Intentamos importar Firebase dinámicamente
        // Si no hay internet, esto fallará y saltará al CATCH
        ...
        onAuthStateChanged(auth, (user) => {
            currentUser = user;
            if (user) {
                updateConnectionStatus(false); // Conectado!
                setupRealtimeScores(collection, onSnapshot);
            }
        });

    } catch (error) {
        console.log("Modo Offline Activado (No se pudo cargar Firebase):", error);
        isOfflineMode = true;
...
function setupRealtimeScores(collectionFunc, onSnapshotFunc) {
    if (!db) return;
    const colRef = collectionFunc(db, 'artifacts', appId, 'public', 'data', 'scores');
    onSnapshotFunc(colRef, (snapshot) => {
        const scores = [];
        ...
        if (!dom.recordsModal.classList.contains('hidden')) {
            renderRecordsList(dom.recordsListView, globalHighScores, "Récords Globales");
        }
    });
}
...
// --- SISTEMA DE PUNTUACIÓN LOCAL Y REMOTA ---
...
// --- OBJETO GLOBAL DEL JUEGO (ACCESIBLE DESDE HTML) ---
window.gameApp = {
    openModeSelection: () => {
    ...
    saveScore: async () => {
        ...
        // Guardado Online (Solo si hay conexión y auth)
        if (!isOfflineMode && db && currentUser) {
            try {
                // Importar dinámicamente las funciones necesarias para guardar si no están cargadas
                ...
                console.warn("No se pudo guardar en la nube (Offline o Error):", e);
            }
        }
        finishSave();
    },
